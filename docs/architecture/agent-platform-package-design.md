...
| **InMemoryState** | §五 | `services/_state.py` | 已实现 |
| **SQLiteStateStore** | §五 | `services/_store.py` | 已实现 |
| **IntentRecognizer** | §十二 链路 | `agents/brain/intent_recognizer.py` | 已实现 |
| **ModeRouter** | §十二 链路 | `agents/brain/mode_router.py` | 已实现 |
| **AgentLoop** | §十二 链路 | `agents/brain/agent_loop.py` | 已实现 |
| **StepExecutor** | §十二 链路 | `agents/brain/step_executor.py` | 已实现 |
| **SafetyEngine** | §十 第一波 | `harness/safety/engine.py` | 已实现 |
| **PolicyEngine** | §十 第一波 | `harness/policy.py` | 已实现 |
| **GuardrailEngine** | §十 第一波 | `harness/guardrails.py` | 已实现 |
| **ConsentStore** | §十 第一波 | `agents/brain/consent_store.py` | 已实现 |
| **EventBus** | §十 第二波 | `harness/hooks.py` | 已实现 |
| **ToolSandbox** | §十 第二波 | `harness/sandbox.py` | 已实现 |
| **CircuitBreaker (重试+熔断)** | §十 第二波 | `agents/brain/circuit_breaker.py` | 已实现 + 已入图 |
| **DelegationTool** | §十 第二波 | `agents/tools/delegation_tools.py` | 已实现 + 已入图 |
| **AgentBudget** | §十 第三波 | `agents/brain/agent_loop.py` | 已实现 |
| **RateLimiter** | §十 第三波 | `services/rate_limiter.py` | 已实现 |
| **AgentCache** | §十 第三波 | `services/agent_cache.py` | 已实现 + 已入图 |
| **MemoryCommitGate** | §十 第一波 | `services/memory_commit_gate.py` | 已实现 |
| **UserProfileService** | §十 第一波 | `services/user_profile_service.py` | 已实现 |
| **BrainContextManager** | §四 | `agents/brain/context_manager.py` | 已实现 |
| **CustomizationEngine** | §十三 | `services/customization_engine.py` | 已实现 |
| **AgentDefManager** | §十三 | `services/agent_def_manager.py` | 已实现 |
| **SkillManager** | §十三 | `services/skill_manager.py` | 已实现 |
| **PromptManager** | §十三 | `services/prompt_manager.py` | 已实现 |
| **McpManager** | §十三 | `services/mcp_manager.py` | 已实现 |
| **FileInstructionManager** | §十三 | `services/file_instruction_manager.py` | 已实现 |
| **InstructionsManager** | §十三 | `services/instructions_manager.py` | 已实现 |
...
| **AgentDefRegistry (双源合并)** | §13.7 | 待实现 (D9) | 已设计 |
| **AgentDefSubAgentAdapter** | §13.6 | 待实现 | 已设计 |
| **AgentOrchestrator** | §13.6 | 待实现 (D12) | 已设计 |
| **SubAgentDef 与 spawn** | §13.6 | 待实现 (D12) | 已设计 |
| **TaskDecomposer (Replan)** | §十一 D11 | 待实现 | 已设计 + 已入图 |
| **ApprovalWorkflow** | §十一 D18 | 待实现 | 已规划 |
| **AgentBus** | §十一 D17 | 待实现 | 已设计 + 已入图 |
| **BudgetPool** | §十一 D23 | 待实现 | 已设计 + 已入图 |
| **CheckpointManager** | §十一 D22 | 待实现 | 已设计 + 已入图 |
| **三层记忆架构** | §十一 D14 | 待实现 | 已设计 + 已入图 |
| **Context Compactor** | §十一 D4 | 待实现 | 已设计 + 已入图 |
| **EvalSuite** | §十一 D19 | 待实现 | 已规划 |

---

### 2.12 多 Agent 协作链路

> 对应 §十二 案例二（文档分析）和案例三（Coding Agent）。覆盖子 Agent 注册、编排、执行、审批、结果合并全流程。
...
| 维度 | 单 Agent (§12.1) | 多 Agent 并行 (§12.2) | 多 Agent 串行 (§12.3) |
|---|---|---|---|
