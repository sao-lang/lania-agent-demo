...
负责把历史模型、查询事件和任务记录转换为新的 runtime contracts 结构，便于 query/task
...
from app.harness.prompting import PromptRenderResult, PromptTemplate
...
def task_memory_entry_to_memory_record(
    ...
    *,
    ...
    """把 `TaskMemoryEntry` 映射为统一 `MemoryRecord`。

    Args:
        entry: 旧任务记忆条目。
        task_id: 所属任务 ID。
        task_run_id: 所属任务运行 ID。
...
def artifact_memory_entry_to_memory_record(
    ...
    *,
    ...
    """把 `ArtifactMemoryEntry` 映射为统一 `MemoryRecord`。
...
def reflection_entry_to_memory_record(
    ...
    *,
    ...
    """把 `ReflectionEntry` 映射为统一 `MemoryRecord`。
...
def query_run_event_to_memory_record(
    ...
    *,
    ...
    """把 query run event 映射为统一 `MemoryRecord`。

    Args:
        event: 原始 query runtime 事件字典。
        run_id: 查询运行 ID。
        collection_name: 所属集合名称。
...
def prompt_template_to_spec(template: PromptTemplate) -> PromptSpec:
    """把 `PromptTemplate` 映射为统一 `PromptSpec`。
...
def build_prompt_build_request(
    *,
    ...
    Args:
        prompt_spec: 提示词规格对象。
        task_spec_ref: 任务规格引用。
        step_spec_ref: 步骤规格引用。
        context_bundle_ref: 上下文包引用。
        tool_specs_ref: 可选工具规格引用列表。
        policy_profile_ref: 可选策略画像引用。
        prompt_profile_ref: 可选提示词画像引用。
...
def prompt_render_result_to_build_result(
    ...
    *,
    ...
    """把 `PromptRenderResult` 映射为统一 `PromptBuildResult`。

    Args:
        render_result: 提示词渲染结果。
        output_contract: 可选输出契约。
        build_notes: 可选构建备注。
...
def build_ad_hoc_prompt_spec(*, prompt_id: str, purpose: str, prompt_text: str, output_schema: str | None = None) -> PromptSpec:
    """为非 PromptBuilder 路径构造一个即席 `PromptSpec`。

    Args:
        prompt_id: 提示词标识。
        purpose: 提示词用途。
        prompt_text: 原始提示词文本。
        output_schema: 可选输出结构描述。
...
def build_ad_hoc_prompt_build_result(
    *,
    ...
    """为非 PromptBuilder 路径构造 `PromptBuildResult`。
...
def evidence_pack_to_grounded_context(
    *,
    ...
    """把 `EvidencePack` 映射为统一 `GroundedContext`。

    Args:
        objective: 当前任务目标。
        evidence_pack: 证据包对象。
        evidence_pack_ref: 证据包引用标识。
        unresolved_gaps: 可选未解决缺口列表。

    Returns:
        可直接挂到 runtime contracts 中的 grounded context。
    """
...
def build_retrieval_quality_report(
    *,
    ...
    """把统一分数映射为 `RetrievalQualityReport`。
...
def _evidence_item_to_fact(item: EvidenceItem) -> dict[str, Any]:
    """把单条证据项转换为 grounded fact 字典。

    Args:
        item: 证据项对象。

    Returns:
        供 grounded context 使用的扁平化事实字典。
    """
    return {
...
def _normalize_datetime(value: Any) -> datetime:
    ...
    Returns:
        如果输入已经是 `datetime` 则直接返回，否则返回当前 UTC 时间。
    """
