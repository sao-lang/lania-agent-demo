...
负责定义文档分析任务从请求、规划、执行、反思到最终结果的全套数据模型，作为任务 API、
任务运行时、工作流节点和存储层之间共享的数据契约。
"""
...
# 任务主状态与运行态共享这一组状态枚举，避免 API、worker、workflow 和持久化层各自发明一套状态名。
...
class RunBudget(BaseModel):
    ...
    该模型属于平台层约束，不直接表达领域语义，而是限制 workflow 在步骤数、单步轮次和工具调用数
    上的最坏开销。
    """
...
class TaskRequest(BaseModel):
    ...
    它是任务系统最外层的入口模型，既表达用户想做什么，也声明权限边界和运行约束。后续 planner、
    workflow 和存储层都会围绕它派生出更细的运行时契约。
    """
...
class StepSpec(BaseModel):
    ...
    与 `TaskStep` 相比，它更偏运行时语义：强调允许哪些工具、最多转几轮、何时视为成功、
    失败时允许采取什么回退动作。
    """
...
class TaskSpec(BaseModel):
    ...
    该模型是任务系统在 workflow 层真正消费的“任务规格书”，由请求、计划或 skill adapter 投影而来。
    """
...
class TaskRun(BaseModel):
    ...
    `TaskSpec` 描述“应该怎么做”，`TaskRun` 描述“这次实际做到哪一步了”。两者组合后，才能完整表达
    一次任务执行的计划和进度。
    """
...
class StepRuntimeRecord(BaseModel):
    ...
    这是步骤粒度的最小运行态单元，专门用来回答“这个步骤跑了几次、何时开始/结束、是否降级、
    为什么退出”。
    """
...
class CheckpointRecord(BaseModel):
    ...
    checkpoint 同时被 query workflow 和 task workflow 复用，因此字段设计保持通用：记录当前步骤、
    下一跳路由、已完成步骤和最小可恢复状态快照。
    """
...
class ReflectionDecision(BaseModel):
    ...
    该模型主要服务于运行态自反思和恢复决策，既能表达“接受/继续”，也能表达“重检索/重规划/保守改写”
    这类带回退动作的分支判断。
    """
...
class TaskRunEvent(BaseModel):
    ...
    这类事件偏向运行审计和恢复，不是前端流式协议；它们通常会被持久化并转译成 memory record。
    """
...
class TaskMemoryEntry(BaseModel):
    ...
    与 `TaskRunEvent` 相比，它更强调面向人和后续智能体可读的摘要视角，而不是单纯运行审计。
    """
...
class TaskDetail(BaseModel):
    ...
    该模型聚合任务请求、计划、执行轨迹、失败信息、产物关联和租约字段，是任务系统内部和
    外部查询都依赖的主状态对象。
    """
    ...
    def ensure_runtime_contracts(self) -> None:
        """确保任务具备平台层 `TaskSpec` / `TaskRun` 契约。

        这是任务详情和 workflow 之间的关键桥接点：不管任务最初来自 API、持久化恢复还是测试构造，
        在真正进入 workflow 前都需要通过这里补齐统一运行态。
        """
    ...
    def _build_runtime_task_spec(self) -> TaskSpec:
        ...
        这里会把请求侧 task_type 路由到对应 skill adapter；若任务已有 planner 产出的 exit criteria，
        也会同步覆盖到平台层成功条件。
        """
    ...
    def _build_task_run_id(self) -> str:
        ...
        `retry_count + 1` 的命名方式保证同一个 task 在多次尝试间拥有可预测的 run 身份。
        """
...
class TaskRunDetail(TaskRunSummary):
    ...
    在 summary 基础上补齐 checkpoint、上下文包、prompt 构建记录和结果契约，用于恢复、排障和审计。
    """
...
class TaskRunReplayRequest(BaseModel):
    """指定从哪个 checkpoint 重放 task runtime。"""
...
class TaskSummaryItem(BaseModel):
    ...
    与 `TaskRunSummary` 不同，这里聚焦的是任务主对象，而不是某一次运行尝试。
    """
