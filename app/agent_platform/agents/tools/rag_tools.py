"""RAG 门面工具模块�?
这组工具只依�?``RagFacade``，为 workflow �?runtime 提供稳定、统一�?RAG 能力入口，避免上层直接耦合更底层的知识检索实现�?"""
...
from app.rag_system.knowledge import (
...
class RagGroundedQueryTool(_BaseRagTool):
    ...
    def run(self, payload: RagGroundedQueryInput, context) -> QueryResponse:
        """把查询输入转换为 ``QueryRequest`` 后交�?facade 执行�?""
