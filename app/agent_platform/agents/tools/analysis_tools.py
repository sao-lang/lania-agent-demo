"""分析类工具模块�?
负责把证据包继续加工成关键发现和风险项，�?Document Analysis Agent 在证据检索之后的主要分析层�?"""
...
from app.agent_platform.agents.tools.base import ToolRetryPolicy
from app.agent_platform.agents.tools.llm_prompting import (
...
class ExtractKeyPointsTool:
    ...
    def run(self, payload: ExtractKeyPointsInput, context) -> ExtractKeyPointsOutput:
        ...
        summary_bits = [
            ...
            f'共抽�?{len(findings)} 条关键发现与 {len(evidence_items)} 条证据�?,
        ]
        ...
        if not risks and payload.evidence_pack.missing_aspects:
            risks.append(
                RiskItem(
                    risk_id='risk-gap-1',
                    title='证据覆盖不足',
                    description=f'当前�?{"�?.join(payload.evidence_pack.missing_aspects[:4])} 的证据覆盖不足，可能造成结论遗漏�?,
                    severity='medium',
                    citation_ids=[],
