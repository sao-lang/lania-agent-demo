...
from app.agent_platform.capabilities.chart import ChartCapability
...
class GenerateChartInput(BaseModel):
    ...
    values: list[float] = Field(description='数据值列�?)
    title: str = Field(default='', description='图表标题')
    xlabel: str = Field(default='', description='X 轴标�?)
    ylabel: str = Field(default='', description='Y 轴标�?)
...
class GenerateChartTool:
    """生成统计图表（柱状图、折线图、饼图、散点图），返回 base64 编码�?PNG 图片�?""
