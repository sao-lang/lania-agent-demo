"""任务记忆服务模块�?
负责在任务工作流中统一读写任务状态、任务记忆、产物版本、反思记录和�?Agent 运行摘要�?并在可用时桥�?`SQLiteStateStore` 做持久化。该模块位于任务编排层下方，�?workflow�?worker �?API 读取任务执行轨迹的共同入口�?"""
...
from app.agent_platform.runtime_contract_adapters import (
...
from app.agent_platform.types import ArtifactRecord, TaskRecord, TaskRunRecord
...
class TaskMemory:
    ...
    def __init__(self, state: InMemoryState, persistence: SQLiteStateStore | None = None) -> None:
        """初始化任务记忆服务�?
        Args:
            state: 进程内状态存储�?            persistence: 可选持久化实现；为空时仅使用内存态数据�?        """
        self.state = state
    ...
    def claim_next_task(self, worker_id: str, lease_seconds: int) -> TaskDetail | None:
        """领取下一条待执行任务�?
        Args:
            worker_id: 当前 worker 标识�?            lease_seconds: 任务租约时长�?
        Returns:
    ...
    def touch_task_heartbeat(self, task_id: str, worker_id: str, lease_seconds: int) -> TaskDetail | None:
        ...
        Args:
            task_id: 任务标识�?            worker_id: 当前 worker 标识�?            lease_seconds: 新租约时长�?
        Returns:
    ...
    def append_memory_record(
        ...
        *,
        ...
        """直接追加一条统一�?MemoryRecord�?
        这是新的活跃写入方法，绕过旧�?TaskMemoryEntry 中间层�?        如果 task_id 为空，则�?record.namespace 中提取�?
        Args:
            record: 已构造好�?MemoryRecord 实例�?            task_id: 可选的任务 ID；为空时�?record.namespace 提取�?
        Returns:
    ...
    def query_memory_records(
        ...
        *,
        ...
        Args:
            task_id: 任务标识�?            scope: 可选过滤范围（working / session / run / semantic / profile）�?            kind: 可选过滤种类�?            trust_level: 可选过滤信任级别�?            limit: 最大返回条数�?
        Returns:
            符合条件�?MemoryRecord 列表，按创建时间倒序�?        """
        task = self.get_task(task_id)
        if task is None:
            return []
    ...
    def append_reflection(
        ...
        *,
        ...
        """记录一次结构化 reflection�?
        Args:
            task_id: 任务标识�?            step: 当前触发 reflection 的步骤名�?            trigger: 触发原因，例�?`review`、`evidence_gap`�?            decision: 本次反思后的决策�?            summary: 面向后续排障的摘要说明�?            missing_aspects: 可选的证据缺口维度�?            missing_sections: 可选的缺失报告字段�?            unsupported_claims: 可选的未被证据支撑的结论�?            review_notes: 可选的审查备注�?            plan_version: 当前计划版本号�?
        Returns:
    ...
    def append_plan_revision(
        ...
        """记录一次局部重规划并可选替换当前计划�?
        Args:
            task_id: 任务标识�?            trigger: 触发重规划的原因类型�?            reason: 触发重规划的详细说明�?            added_steps: 本次新增的步骤标识列表�?            plan: 可选的新计划对象；提供时会替换当前计划�?
        Returns:
            任务存在时返回更新后的任务详情，否则返回 `None`�?        """
    ...
    def record_tool_call(
        ...
        """记录工具调用历史�?
        Args:
            task_id: 任务标识�?            tool_call_id: 工具调用唯一标识�?            tool_name: 工具名称�?            status: 调用结果状态�?            duration_ms: 调用耗时�?            input_preview: 输入摘要�?            output_summary: 可选输出摘要�?            error: 可选错误信息�?            step: 可选步骤名称�?            error_type: 可选错误类型�?            default_action: 可选默认回退动作�?            retry_count: 当前调用重试次数�?
        Returns:
            任务存在时返回更新后的任务详情，否则返回 `None`�?        """
    ...
    def record_sub_agent_run(
        ...
        *,
        ...
        """记录一次受控子代理执行摘要�?
        Args:
            task_id: 任务标识�?            agent_name: �?Agent 名称�?            action: 执行动作名称�?            status: 本次执行状态�?            allowed_tools: 允许使用的工具白名单�?            selected_tools: 本次实际使用的工具列表�?            input_summary: 输入摘要�?            output_summary: 输出摘要�?
        Returns:
            任务存在时返回更新后的任务详情，否则返回 `None`�?        """
    ...
    def reset_for_retry(self, task_id: str) -> TaskDetail | None:
        ...
        if task is None:
            return None
        # 保留既有 artifact，便于人工回溯历史版本，但把运行态指标与中间记忆全部清空�?        task.status = 'queued'
        task.plan = None
