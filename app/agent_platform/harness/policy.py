"""Policy Harness 实现�?
负责把任务约束从隐式经验规则抽成显式策略判断，并支持从外�?yaml 配置加载策略画像�?�?phase3 中，PolicyEngine 保留兼容 facade，内部拆�?profile store、resolver、evaluator�?"""
...
class PolicyEngine:
    ...
    def check_agent_tool(
        ...
        """校验工具是否�?Agent 白名单内�?
        Args:
            tool_name: 工具名称�?            agent: Agent 定义（含 allowed_tools）�?            allowed_tools: 直接传入的白名单（优先）�?
        Returns:
        ...
        # None = 未设置限�?        if tool_list is None:
        ...
        if tool_name not in tool_list:
            return PolicyDecision(
                ...
                stage="agent_tool_check",
                policy_name="agent_allowed_tools",
                reason=f"Agent 不允许使用工�?'{tool_name}'",
    ...
    def evaluate(
        ...
        """Brain 路径的统一策略评估入口�?
        对当前执行模式做多层策略检查：
        1. Agent 工具白名�?        2. 用户角色权限（预留）

        Args:
            tool_name: 工具名称�?            tool_args: 工具参数（预留）�?            user_id: 用户 ID（预留）�?            role: 用户角色（预留）�?
        Returns:
            PolicyDecision 决策结果�?        """
        # 1. Agent 工具白名单检�?        result = self.check_agent_tool(tool_name, allowed_tools=None)
