...
负责收口核心 prompt，建立统一渲染入口和版本管理能力，支持�?regression eval 打通�?"""
...
from app.agent_platform.harness.prompt_registry import PromptVersionRegistry
from app.agent_platform.harness.models import ContextBundle
from app.agent_platform.harness.policy import PolicyProfile
...
class PromptBuilder:
    ...
    def _register_default_templates(self) -> None:
        ...
        templates = [
            PromptTemplate(
                ...
                content='你是 Document Analysis Agent 的分析模块。请只输出严�?JSON，不要输出额外解释、Markdown 或代码块。\n'
                        '输出 schema: {schema}\n'
                        '规则:\n'
                        '1. 只能输出有证据支撑的结论。\n'
                        '2. citation_ids 必须来自给定证据编号。\n'
                        '3. summary 需覆盖任务目标、主要发现和证据缺口。\n'
                        '4. confidence 必须�?0 �?1 之间。\n'
                        '用户任务: {objective}\n'
                        '文档上下�?\n{documents}\n'
                        '证据列表:\n{evidence}\n'
            ...
            PromptTemplate(
                ...
                content='你是 Document Analysis Agent 的风险抽取模块。请只输出严�?JSON，不要输出额外解释。\n'
                        '输出 schema: {schema}\n'
                        '规则:\n'
                        '1. 只保留高价值、可行动的风险。\n'
                        '2. citation_ids 必须来自给定证据编号。\n'
                        '3. severity 只能�?low/medium/high。\n'
                        '审查目标: {objective}\n'
                        '证据:\n{evidence}\n'
                        '证据缺口: {missing_aspects}',
            ),
            PromptTemplate(
                ...
                content='你是 Document Analysis Agent 的报告生成模块。请只输出严�?JSON，不要输出额外解释。\n'
                        '输出 schema: {schema}\n'
                        '规则:\n'
                        '1. 报告必须基于提供的分析结果和证据。\n'
                        '2. 所有结论必须有证据支撑。\n'
            ...
            PromptTemplate(
                ...
                content='你是 Document Analysis Agent 的质量审查模块。请只输出严�?JSON，不要输出额外解释。\n'
                        '输出 schema: {schema}\n'
                        '规则:\n'
                        '1. unsupported_claims 必须填写 finding_id �?risk_id。\n'
    ...
    def _format_missing_aspects(self, context: ContextBundle) -> str:
        ...
        return ', '.join(missing_aspects) if missing_aspects else '�?
